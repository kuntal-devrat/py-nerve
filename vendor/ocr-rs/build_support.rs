...
pub fn select_link_mode<'a>(
    ...
    // A locally packaged prebuilt MNN (built with the SIMD flags we want)
    // skips the cmake compile entirely. The directory must contain `include/`
    // and `lib/`, mirroring the layout of downloaded prebuilts.
    if let Ok(prebuilt_dir) = env::var("OCR_RS_MNN_PREBUILT_DIR") {
        let dir = PathBuf::from(prebuilt_dir);
        if dir.join("include").is_dir() && dir.join("lib").is_dir() {
            return Ok(MnnLinkMode::Prebuilt);
        }
        panic!(
            "OCR_RS_MNN_PREBUILT_DIR is set to {} but it must contain include/ and lib/",
            dir.display()
        );
    }
