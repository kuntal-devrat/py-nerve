...
fn main() {
    ...
    // Allow toggling between a packaged prebuilt MNN and a from-source build
    // without touching the crate sources.
    println!("cargo:rerun-if-env-changed=OCR_RS_MNN_PREBUILT_DIR");
    ...
    let (mnn_include_dir, mnn_lib_dir) = match &link_mode {
        MnnLinkMode::Prebuilt => {
            // A locally packaged prebuilt (built once with our SIMD flags and
            // uploaded as a release artifact) completely skips cmake.
            let (include_dir, lib_dir): (PathBuf, PathBuf) =
                if let Ok(prebuilt_dir) = env::var("OCR_RS_MNN_PREBUILT_DIR") {
                    let dir = PathBuf::from(prebuilt_dir);
                    let inc = dir.join("include");
                    let lib = dir.join("lib");
                    if !inc.exists() || !lib.exists() {
                        panic!(
                            "OCR_RS_MNN_PREBUILT_DIR set but include/ or lib/ missing in {}",
                            dir.display()
                        );
                    }
                    println!(
                        "cargo:warning=Using packaged prebuilt MNN from {}",
                        dir.display()
                    );
                    (inc, lib)
                } else {
...
fn build_mnn_with_cmake(
    ...
    // SIMD optimizations
    //
    // MNN's CPU backends compile in every kernel family listed here and then
    // auto-detect the host CPU at runtime (cpuid on x86, feature probes on
    // ARM), selecting the fastest available implementation
    // (AVX512 > AVX2 > AVX > SSE; NEON on ARM). Disabling a family here would
    // silently cap every machine at the lowest level, so we enable them all
    // and let MNN's runtime detection do the rest.
    if arch == "x86_64" && os != "android" && os != "ios" {
        config.define("MNN_USE_SSE", "ON");
        config.define("MNN_USE_AVX", "ON");
        config.define("MNN_USE_AVX2", "ON");
        config.define("MNN_USE_AVX512", "ON");
    } else {
    ...
    // ARM: NEON is the baseline SIMD for ARM64; MNN_ARM82 additionally
    // compiles ARMv8.2 (fp16) kernels, gated by runtime feature detection.
    if arch == "aarch64" {
        config.define("MNN_USE_NEON", "ON");
...
fn build_wrapper(
        ...
        .unwrap_or_else(|error| panic!("Invalid C++ target configuration: {}", error))
    {
        ...
        // Match CRT with the MNN library: the upstream downloaded prebuilts are
        // /MT (static CRT), but a packaged prebuilt built from source (our
        // OCR_RS_MNN_PREBUILT_DIR) is /MD like the from-source build, so the
        // wrapper must use the same CRT or the linker rejects the mismatch.
        if matches!(link_mode, MnnLinkMode::Prebuilt) && env::var("OCR_RS_MNN_PREBUILT_DIR").is_err() {
